...
module.exports = {
     ...
     * @returns {Promise} - the create operation.
     */
    user: function (fb, users) {
        let email = `testuser-${chance.word({length: 8})}@nature-net.org`;
        ...
        .then(auth => {
            // return random_avatar().then(url => {
                return users.child(auth.uid).set({
                    display_name: `${chance.word()} ${chance.word()}`,
                    email: email,
                    avatar_url: chance.pick(activity_icons),
