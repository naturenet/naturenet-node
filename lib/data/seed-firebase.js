...
last = last.then(ok => {
    return Ideas.newRecord({
        submitter: testusers[0].id,
        group: "Wouldn't it be cool if...",
        icon_url: 'http://res.cloudinary.com/university-of-colorado/image/upload/v1427400563/2_Snow_rutfs8.png',
        content: "ArgleBargle"
    }).write().then(idea => { testideas.push(idea); });
})

last = last.then(ok => {
    return Promise.all([
        testideas[0].addComment("Its ok", testusers[0]),
        testobservations[0].addComment("alot of ducks right?", testusers[0])
    ]);
})
.then(ok => {
