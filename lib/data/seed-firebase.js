...
function mockActivity() {
    ...
    return Activities.newRecord({
        ...
        status: chance.pick(['Open', 'Completed']),
        template: {
...
/**
* Creates a single User Group with randomized data.
*/
function mockGroup() {
    return Groups.newRecord({
        name: chance.word() + ' ' + chance.word(),
        avatar: 'https://s3.amazonaws.com/uifaces/faces/twitter/robertovivancos/128.jpg',
        description: chance.sentence(),
        affiliation: chance.pick([null, 'ACES', 'NatureNet']),
        owner: chance.pick(mockUsers).id()
    }).write().then(group => { mockGroups.push(group) });
}
...
function mockObservation() {
    ...
    return activity.locations().then(list => {
        ...
        return Observations.newRecord({
            ...
            location: locationNear(location.l, 2),
            data: {
                image: chance.pick(activityIcons),
...
function mockIdea() {
    let user = chance.pick(mockUsers);
    let group = chance.pick(["Idea", "Challenge", "something else"]);
    return Ideas.newRecord({
        ...
        status: chance.pick(["Open","Done","Closed"])
...
/**
 * Adds a random member to a random user group
 */
function randomlyJoin(params) {
    let member = chance.pick(mockUsers);
    let joinable = chance.pick(mockGroups);
    return joinable.join(member);
}
...
authenticate()
    ...
    .then(ok => repeat(10, mockGroup))
    .then(ok => repeat(40, randomlyJoin))
