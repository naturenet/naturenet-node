...
/**
 * Mixes in membership functionality to any other class. Stores members as a nested
 * object under the 'members' key in the parent. The structure of the object will
 * be as follows:
 *
 * @example
 * members: {
 *      // this user is a member of the record
 *      "abdcefg-12345": true
 * }
 */
const Joinable = (RecordClass) => class extends RecordClass {

    /**
     * Add a new member to the record.
     *
     * @param {User} user - The user who is joining.
     * @return {Promise} A promise for the write operation.
     */
    join(user) {
        let membersRef = this.ref.child('members');
        let data = {};
        data[user.id] = true;
        return membersRef.update(data)
            .then(ok => {
                if(!this.members) {
                    this.members = {};
                }
                this.members[user.id] = true;
                return this;
            });
    }
}
